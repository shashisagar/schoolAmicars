...
@section('title')
    {{trans('message.ManageMainStudent')}}
@endsection
...
@section('menuLink')
    <ol class="breadcrumb">
        <li>{{trans('message.MainStudent')}}</li>
        <span class="userDefineArrow"></span>
    </ol>
    <ol class="breadcrumb">
        <li>{{trans('message.ViewMainStudent')}}</li>
    </ol>
@endsection
        ...
        <div class="col-md-12">
            <div class="panel panel-white">
                <div class="panel-body">
                    <h2>{{trans('message.MainStudentList')}}</h2><br>
                    <table id="studentList" class="display nowrap" cellspacing="0" width="100%">
                        <thead>
                        <tr>
...
//            $('#student').addClass('open');
            ...
            $('#studentList').DataTable({
                ...
                "ajax": {
                    url: "/student-ajax-handler",
                    type: 'POST',
                    data: {
                        method: 'allStudentList',
                    }
                },
                columns: [
...
//                    {data: 'schools_id', name: 'users.schools_id'},
            ...
            $(document.body).on('click', '.delete-student', function () {
                var obj = $(this);
                var userId = $(this).attr('data-userId');
                if (confirm("<?php echo trans('message.DoyouwanttodeletethisMainStudent!') ?>") == true) {
                    $.ajax({
                        url: '/student-ajax-handler',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            method: 'deleteMainStudent',
                            userId: userId,
                        },
                        success: function (response) {
                            if (response) {
                                toastr[response['status']](response['msg']);
                                $('#studentList').DataTable().ajax.reload(null, false);
                            }
                        }
                    });
                }
